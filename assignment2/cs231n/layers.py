...
def softmax_loss(x, y):
	...
	if np.any(exp_sums==np.inf):
		print('inf')
	if np.any(exp_sums==-np.inf):
		print('-inf')
	if np.any(exp_sums==0):
		print('0')
	

	softmax=exps/exp_sums
	loss=np.mean(-np.log(softmax[xi, y]))
...
def batchnorm_backward(dout, cache):
	...
	#x gradient
	dl_dxhat=dout*gamma
	dl_dvar=np.sum(dl_dxhat*(x-mu), axis=0)*(-.5)*(var+eps)**(-3/2)
	dl_dmu=np.sum(-dl_dxhat/np.sqrt(var+eps), axis=0)+dl_dvar*np.mean(-2*(x-mu), axis=0)
