...
class FullyConnectedNet(object):
	...
	def loss(self, X, y=None):
		...
		for i in range(1, self.num_layers+1):
			#affine forward
			scores, caches[f'affine{i}']=affine_forward(
				scores,
				self.params[f'W{i}'],
				self.params[f'b{i}']
			)
			...
			#compute activation function if not on last layer
			if i!=self.num_layers:
				if self.normalization == 'batchnorm':
					scores, caches[f'bnorm{i}']=batchnorm_forward(
						scores,
		...
		loss+=.5*self.reg*np.sum(np.array([np.sum(self.params[f'W{i}']**2) for i in range(1, self.num_layers+1)]))
