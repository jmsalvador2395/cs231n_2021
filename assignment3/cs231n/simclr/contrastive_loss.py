...
def sim(z_i, z_j):
	...
	norm_dot_product = z_i@z_j / (
		torch.linalg.vector_norm(z_i) * \
		torch.linalg.vector_norm(z_j)
	)
...
def simclr_loss_naive(out_left, out_right, tau):
	...
	for k in range(N):	# loop through each positive pair (k, k+N)
		...
		def l(a, b, k):
			return -torch.log(
				torch.exp(sim(a, b)/tau) / 
				torch.sum(torch.exp(
					torch.stack([sim(a, out[zk]) for zk in range(2*N) if zk != k])/tau
				))
			)
		total_loss += l(z_k, z_k_N, k) + l(z_k_N, z_k, k+N)
...
def sim_positive_pairs(out_left, out_right):
	...
	norm_left = torch.linalg.vector_norm(
		out_left,
		dim=-1,
		keepdim=True
	)
	norm_right = torch.linalg.vector_norm(
		out_right,
		dim=-1,
		keepdim=True
	)

	norm_prod = norm_left*norm_right

	pos_pairs = out/norm_prod
...
def compute_sim_matrix(out):
	...
	norms = norms.outer(norms)
	sim_matrix = dots/norms
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
	...
	pos_pair = sim_positive_pairs(out_left, out_right).repeat((2, 1))
