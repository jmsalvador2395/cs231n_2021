...
class CaptioningRNN:
	...
	def loss(self, features, captions):
		...
		# get word embeddings
		out, cache['W_embed'] = word_embedding_forward(captions_in, W_embed)
		...
		out, cache['temporal_affine'] = temporal_affine_forward(
			out,
			W_vocab,
			b_vocab,
		)
		...
		dout, dw, db  = temporal_affine_backward(dout, cache['temporal_affine'])
		grads['W_vocab'] = dw
		grads['b_vocab'] = db
		...
		grads['W_embed'] = word_embedding_backward(dout_x, cache['W_embed'])
		dout, grads['W_proj'], grads['b_proj'] = affine_backward(dout_h0, cache['h0'])
	...
	def sample(self, features, max_length=30):
		...
		for t in range(max_length):
			...
			# save preds to captions
			captions[:, t] = preds

			# get next word embeddings
			x, _ = word_embedding_forward(preds[:, None], W_embed)
			x = x[:, 0, :]
